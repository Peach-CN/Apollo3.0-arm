...
def cuda_library_static_impl(ctx):
    flags = ' '.join(ctx.attr.flags)
    output = ctx.outputs.out
    lib_flags = ["-std=c++11", "-lib", "--compiler-options -fPIC"]
    args = [f.path for f in ctx.files.srcs]
...
def cuda_binary_impl(ctx):
    ...
    ctx.actions.run_shell(
            ...
            arguments=args,
            env={ 'PATH':'/usr/local/cuda/bin:/usr/local/bin:/usr/bin:/bin', },
            command="/usr/local/cuda/bin/nvcc %s %s %s -o %s" % (' '.join(cuda_arch), " ".join(args), " ".join(deps_flags), output.path),
     )
...
cuda_library_static = rule(
    ...
    implementation = cuda_library_static_impl,
)
