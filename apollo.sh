...
function build() {
  ...
  if [ "$MACHINE_ARCH" == 'aarch64' ]; then
    JOB_ARG="--jobs=8 --local_resources 16384,8,1.0"
  fi
  info "Building with $JOB_ARG for $MACHINE_ARCH"

  # Switch for building fuzz test.
  if [ -z $BUILD_FUZZ_TEST ]; then
    echo "$BUILD_TARGETS" | xargs bazel build --action_env=LD_LIBRARY_PATH --action_env=PATH $JOB_ARG $DEFINES -c $@
  else
...
function cibuild() {
  ...
  JOB_ARG="--jobs=$(nproc)"
  if [ "$MACHINE_ARCH" == 'aarch64' ]; then
    JOB_ARG="--jobs=8 --local_resources 16384,6,1.0"
  fi
  info "Building with $JOB_ARG for $MACHINE_ARCH"
  ...
  //modules/routing/...
  "
  bazel build --action_env=LD_LIBRARY_PATH --action_env=PATH $JOB_ARG $DEFINES $@ $BUILD_TARGETS
