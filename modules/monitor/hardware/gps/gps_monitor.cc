...
DEFINE_string(imu_hardware_name, "IMU", "Name of the IMU hardware.");
...
void GpsMonitor::RunOnce(const double current_time) {
  ...
  static auto *imustatus = MonitorManager::GetHardwareStatus(
      FLAGS_imu_hardware_name);
  ...
  if (imu_adapter->Empty()) {
    imustatus->set_status(HardwareStatus::ERR);
    imustatus->set_detailed_msg("No Imu message.");
    bFlag[1] = false;
  } else {
    double message_delay = imu_adapter->GetDelaySec();
    if (message_delay < 0 || message_delay > 1.0) {
      imustatus->set_status(HardwareStatus::ERR);
      imustatus->set_detailed_msg("Imu message delay more than 1s.");
      bFlag[1] = false;
    }
  }
  if (bFlag[1]) {
    imustatus->set_status(HardwareStatus::OK);
    imustatus->set_detailed_msg("OK");
  }
  ...
  if (current_timestamp - gnss_status_adapter->GetLatestObserved().header().timestamp_sec() < 1. &&
      !gnss_status_adapter->GetLatestObserved().solution_completed()) {
