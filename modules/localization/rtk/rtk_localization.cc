...
DEFINE_double(orientation_revised, 3.1415926 / 2, "Adjust heading");
...
Status RTKLocalization::Start() {
  ...
  if (FLAGS_enable_ekf_localization) {
    ekf_estimator_.reset(new EKFEstimator());
    if ( !ekf_estimator_ ) {
      return Status(common::LOCALIZATION_ERROR,
                    "localization with Extended Kalman Filter is not initialized" );
    }
    ...
    //ekf_estimator_->SetInitialState(p_0, Eigen::Vector3d::Zero(), Init_T);
    ekf_estimator_->Start();
  }
...
void RTKLocalization::PrepareLocalizationMsg(
  ...
  if (FLAGS_enable_ekf_localization) {
#if 0
    Eigen::Vector3d accl((*accelerometer_vec_)(i, 0),
                        (*accelerometer_vec_)(i, 1),
                        (*accelerometer_vec_)(i, 2));
    Eigen::Vector3d gyro((*gyro_vec_)(i, 0), (*gyro_vec_)(i, 1), (*gyro_vec_)(i, 2));
    ekf_estimator_->UpdateWithInertialMeasurement(accl, ACCELEROMETER);
    ekf_estimator_->UpdateWithInertialMeasurement(gyro, GYRO);
    std::vector<Eigen::Matrix<double, 6, 1>> gps_data;
    gps_data.push_back((*gps_vec_).row(i).transpose());
    ekf_estimator_->UpdateWithGPSMeasurements(gps_data);
#endif
  }
...
void RTKLocalization::ComposeLocalizationMsg(
  ...
  if (gps_msg.has_localization()) {
    ...
    if (pose.has_orientation()) {
      ...
      mutable_pose->set_heading(heading + FLAGS_orientation_revised);
    }
    // linear velocity
  ...
  if (FLAGS_enable_ekf_localization) {
    Eigen::Vector3d position = ekf_estimator_->GetPositionState();
    AINFO << "x = " << position(0) * 180.0 / M_PI << "\ty = " << position(1) * 180.0 / M_PI;
    Eigen::Vector3d velocity = ekf_estimator_->GetVelocityState();
    AINFO << "mps = " << std::sqrt(velocity(0) * velocity(0) + velocity(1) * velocity(1));
    Eigen::Matrix3d orientation = ekf_estimator_->GetOrientationState();
    double azimuth = ekf_estimator_->GetAzimuth();
