...
void Convert::init(ros::NodeHandle& node, ros::NodeHandle& private_nh) {
  ...
  private_nh.param("use_gps_time", config_.use_gps_time, false);
  private_nh.param("slam_topic", topic_slam_localization_, std::string("/lslidar_point_cloud"));
  ...
  pointcloud_localization_pub_ = 
      node.advertise<sensor_msgs::PointCloud2>(topic_slam_localization_, queue_size_);
...
void Convert::convert_packets_to_pointcloud(
  ...
  if (!config_.use_gps_time) {
    pointcloud->header.stamp = static_cast<uint64_t>(ros::Time::now().toSec() * 1e6);
  }
  // publish the accumulated cloud message
  pointcloud_pub_.publish(pointcloud);
  pointcloud_localization_pub_.publish(pointcloud);
}
