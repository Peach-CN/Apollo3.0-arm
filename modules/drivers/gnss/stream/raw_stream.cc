...
void RawStream::DataSpin() {
  ...
  while (ros::ok()) {
    ...
    if (length > 0) {
      char *gpgga = static_cast<char *>(memchr(reinterpret_cast<void *>(buffer_), '$', length));
      if (gpgga) {
        buffer_record_flag_ = true;
      }
      if (buffer_record_flag_ && recv_data_length_ + length < sizeof(buffer_gga_)) {
        memcpy(&buffer_gga_[recv_data_length_], buffer_, length);
        recv_data_length_ += length;
      } else if (buffer_record_flag_) {
        recv_data_length_ = 0;
        memset(&buffer_gga_[0], 0, sizeof(buffer_gga_));
      }
...
#else
  char *gpgga = static_cast<char *>(memchr(reinterpret_cast<void *>(buffer_gga_), '$', recv_data_length_));
  if (gpgga && !memcmp(gpgga, "$GPGGA", 6)) {
    char *p = static_cast<char *>(memchr(reinterpret_cast<void *>(gpgga), '*', 
        recv_data_length_ - size_t(gpgga - reinterpret_cast<char *>(buffer_gga_))));
    if (p) {
      p += 5;
      if (size_t(p - reinterpret_cast<char *>(buffer_gga_)) <= recv_data_length_) {
        AINFO_EVERY(5) << "Push gpgga.";
        in_rtk_stream_->write(reinterpret_cast<uint8_t *>(gpgga), p - gpgga);
        buffer_record_flag_ = false;
        recv_data_length_ -= size_t(p - reinterpret_cast<char *>(buffer_gga_));
        if (recv_data_length_ > 0) {
          memmove(&buffer_gga_[0], p, recv_data_length_);
        }
        memset(&buffer_gga_[recv_data_length_], 0, 
               sizeof(buffer_gga_) - recv_data_length_);
      }
    }
  }
#endif
