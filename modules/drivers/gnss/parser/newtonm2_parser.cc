...
NewtonM2Parser::NewtonM2Parser(const config::Config& config) {
  ...
  if(FLAGS_use_100d2imu)
  {  
    node_handle_.reset(new ros::NodeHandle("LaserScanRecv"));
    imu100d2_sub_ = node_handle_->subscribe("/imu_data", 10,
                        &NewtonM2Parser::Imu100d2Callback, static_cast<NewtonM2Parser *>(this));
  } 
...
void NewtonM2Parser::Imu100d2Callback(const sensor_msgs::Imu& msg)
{
    ...
    ins_.mutable_linear_acceleration()->set_x(msg.linear_acceleration.x);
    ins_.mutable_linear_acceleration()->set_y(msg.linear_acceleration.y);
    ins_.mutable_linear_acceleration()->set_z(msg.linear_acceleration.z);
    Eigen::Quaterniond q(msg.orientation.w,msg.orientation.x,msg.orientation.y,msg.orientation.z);

    Eigen::Vector3d eulerAngle=q.toRotationMatrix().eulerAngles(0,1,2);
    if(FLAGS_use_100d2imu_euler)
    {
...
bool NewtonM2Parser::HandleInsCov(const novatel::InsCov* cov) {
  for (int i = 0; i < 9; ++i) {
    ...
    ins_.set_euler_angles_covariance(
        newtonm2::INDEX[i], (newtonm2::DEG_TO_RAD * newtonm2::DEG_TO_RAD) *
...
bool NewtonM2Parser::HandleInsPva(const novatel::InsPva* pva) {
  ...
  if(!FLAGS_use_100d2imu_euler)
  {
    ins_.mutable_euler_angles()->set_x(pva->roll * newtonm2::DEG_TO_RAD);
    ins_.mutable_euler_angles()->set_y(-pva->pitch * newtonm2::DEG_TO_RAD);
