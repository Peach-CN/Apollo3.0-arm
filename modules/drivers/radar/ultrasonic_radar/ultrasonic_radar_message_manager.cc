...
void UltrasonicRadarMessageManager::Parse(const uint32_t message_id,
  ...
  if (message_id == 0x301) {
    sensor_data_.set_ranges(2, data[1] * 2);
    sensor_data_.set_ranges(1, data[2] * 2);
    sensor_data_.set_ranges(9, data[3] * 2);
    sensor_data_.set_ranges(3, data[4] * 2);
  } else if (message_id == 0x302) {
    sensor_data_.set_ranges(4, data[1] * 2);
    sensor_data_.set_ranges(5, data[2] * 2);
    sensor_data_.set_ranges(6, data[3] * 2);
    sensor_data_.set_ranges(7, data[4] * 2);
  } else if (message_id == 0x303) {
    sensor_data_.set_ranges(8, data[1] * 2);
    sensor_data_.set_ranges(0, data[2] * 2);
  } else if (message_id == 0x304) {
    sensor_data_.set_ranges(10, data[1] * 2);
    sensor_data_.set_ranges(11, data[2] * 2);
