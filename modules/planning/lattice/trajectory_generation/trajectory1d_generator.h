...
class Trajectory1dGenerator {
 ...
 private:
  ...
  template <std::size_t N>
  void GenerateTrajectory1DBundleStop(
      const std::array<double, 3>& init_state_stop,
      const std::vector<std::pair<std::array<double, 3>, double>>&
          end_conditions_stop,
      std::vector<std::shared_ptr<Curve1d>>* ptr_trajectory_bundle_stop) const;
...
template <>
inline void Trajectory1dGenerator::GenerateTrajectory1DBundleStop<5>(
    const std::array<double, 3>& init_state_stop,
    const std::vector<std::pair<std::array<double, 3>, double>>& end_conditions_stop,
    std::vector<std::shared_ptr<Curve1d>>* ptr_trajectory_bundle_stop) const {
  CHECK_NOTNULL(ptr_trajectory_bundle_stop);
  CHECK(!end_conditions_stop.empty());

  ptr_trajectory_bundle_stop->reserve(end_conditions_stop.size());
  for (const auto& end_condition : end_conditions_stop) {
    auto ptr_trajectory1d = std::make_shared<LatticeTrajectory1d>(
        std::shared_ptr<Curve1d>(new QuarticPolynomialCurve1d(
            init_state_stop, {end_condition.first[1], end_condition.first[2]},
            end_condition.second)));
    auto speed = end_condition.first[1];
    if (speed < 0.1) {
        ptr_trajectory1d->set_target_velocity(0.0);
        ptr_trajectory1d->set_target_time(end_condition.second);
        ptr_trajectory_bundle_stop->push_back(ptr_trajectory1d);
    }
    ptr_trajectory1d->set_target_velocity(end_condition.first[1]);
    ptr_trajectory1d->set_target_time(end_condition.second);
    ptr_trajectory_bundle_stop->push_back(ptr_trajectory1d);
