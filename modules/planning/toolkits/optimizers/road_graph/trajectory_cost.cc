...
ComparableCost TrajectoryCost::GetCostFromObsSL(
  ...
  const double kSafeDistance = 0.1;
  ...
  if(adc_left_l > 1.4)
    obstacle_cost.safety_cost +=
      config_.obstacle_collision_cost() *
      Sigmoid(config_.obstacle_collision_distance() - std::fabs(1.4 - adc_left_l));

  if(adc_right_l < -1.4)
    obstacle_cost.safety_cost +=
      config_.obstacle_collision_cost() *
      Sigmoid(config_.obstacle_collision_distance() - std::fabs(adc_right_l + 1.4));
#endif
  return obstacle_cost;
}
