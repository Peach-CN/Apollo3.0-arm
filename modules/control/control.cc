...
Status Control::Init() {
  ...
  ros::NodeHandle cmdh;
  lsc_cmdpub = cmdh.advertise<geometry_msgs::Twist>("/cmd_vel", 1, true);
...
Status Control::CheckInput() {
  ...
  {
    AINFO << "00000000000000000000000000000000";
  }
  if (trajectory_.trajectory_point_size() != 0 && estop_ == true)
  {
    estop_ = false;
  }
  /////////////////
...
void Control::SendCmd(ControlCommand *control_command) {
  ...
  twist.linear.x = control_command->throttle();
  twist.angular.z = control_command->steering_target();

  lsc_cmdpub.publish(twist);
}
