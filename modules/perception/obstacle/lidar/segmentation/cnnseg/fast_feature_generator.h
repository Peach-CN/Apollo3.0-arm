...
#ifndef MODULES_PERCEPTION_OBSTACLE_LIDAR_SEGMENTATION_FAST_CNNSEG_FEATURE_GENERATOR_H_  // NOLINT
#define MODULES_PERCEPTION_OBSTACLE_LIDAR_SEGMENTATION_FAST_CNNSEG_FEATURE_GENERATOR_H_  // NOLINT
...
class FastFeatureGenerator {
 public:
  FastFeatureGenerator() {}

  ~FastFeatureGenerator() {}

  bool Init(const FastFeatureParam& fast_feature_param, caffe::Blob<Dtype>* out_blob);

  void Generate(apollo::perception::pcl_util::PointCloudConstPtr pc_ptr);

  void generate_gpu(const apollo::perception::pcl_util::PointCloudConstPtr& pc_ptr);

  inline std::string name() const { return "FastFeatureGenerator"; }
 ...
 private:
  ...
  int _data_channel;
  bool _use_intensity_feature = true;

  int _pc_gpu_size = 0;
  pcl_util::Point *_pc_gpu = nullptr;
  boost::shared_ptr<caffe::Blob<Dtype>> _log_blob;
};
