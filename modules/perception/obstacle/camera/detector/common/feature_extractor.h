...
#include "rt_net/include/blob.h"
#include "rt_net/include/layer.h"
...
class RTNetBaseFeatureExtractor {
public:
	RTNetBaseFeatureExtractor() = default;
	virtual ~RTNetBaseFeatureExtractor() = default;
	virtual bool init(const ExtractorParam &param,
		const std::shared_ptr<base::Blob<float>> feat_blob,
		int input_width = 0, int input_height = 0) = 0;
...
private:
	DISALLOW_COPY_AND_ASSIGN(RTNetBaseFeatureExtractor);
};
...
class RTNetReorgFeatureExtractor : public RTNetBaseFeatureExtractor {
public:
	bool init(const ExtractorParam &param,
		const std::shared_ptr<base::Blob<float>> feat_blob,
		int input_width = 0, int input_height = 0) override;
...
protected:
	std::vector<std::shared_ptr<base::Blob<float>>> bottom_vec_;
	std::vector<std::shared_ptr<base::Blob<float>>> top_vec_;
	std::shared_ptr<base::Blob<float>> reorg_feat_blob_;
	std::shared_ptr<inference::Layer<float>> reorg_layer_ = nullptr;
	bool skip_reorg_ = false;
	int ref_height_ = 0;
	int ref_width_ = 0;
};
...
class RTNetROIPoolingFeatureExtractor : public RTNetBaseFeatureExtractor {
public:
	bool init(const ExtractorParam &param,
		const std::shared_ptr<base::Blob<float>> feat_blob,
		int input_width = 0, int input_height = 0) override;
...
protected:
	std::vector<std::shared_ptr<base::Blob<float>>> bottom_vec_;
	std::vector<std::shared_ptr<base::Blob<float>>> top_vec_;
	std::shared_ptr<base::Blob<float>> rois_blob_;
	std::shared_ptr<base::Blob<float>> roi_feat_blob_;
	std::shared_ptr<inference::Layer<float>> roi_pooling_layer_ = nullptr;
	int input_height_ = 0;
	int input_width_ = 0;
};
}  // namespace perception
