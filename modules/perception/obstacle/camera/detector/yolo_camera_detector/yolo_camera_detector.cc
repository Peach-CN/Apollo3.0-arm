...
void YoloCameraDetector::load_intrinsic(
  ...
  if (options.intrinsic == nullptr) {
    ...
    image_height_ = FLAGS_input_image_height/*1080*/;
    image_width_ = FLAGS_input_image_width/*1920*/;
  } else {
    image_height_ = options.intrinsic->get_height();
    image_width_ = options.intrinsic->get_width();
  }

  if (FLAGS_input_image_width != 1920) {
    offset_y_ = 0;
    width_ = FLAGS_inference_blob_input_width;
    height_ = FLAGS_inference_blob_input_height;
    image_width_ = FLAGS_inference_blob_input_width;
   image_height_= FLAGS_inference_blob_input_height;
  } else {
...
bool YoloCameraDetector::Detect(
  ...
  if (1920 == frame.cols && 1080 == frame.rows) {
    ...
    double ratio = width_ * 1. / frame.cols;
    int offset_y = (frame.rows - static_cast<int>(frame.rows * ratio)) / 2;
    cv::Mat img(width_, height_, CV_8UC3);
    cv::Rect roi(0, offset_y, frame.cols, static_cast<int>(frame.rows * ratio));
    cv::resize(frame(roi), img, cv::Size(width_, height_), 0, 0);
    const auto input_blob =
            cnnadapter_->get_blob_by_name(yolo_param_.net_param().input_blob());
    resize(img, input_blob.get(), image_data_, 0);
  }
  pre_time.Stop();
  AINFO << "Pre-processing: " << pre_time.MilliSeconds() << " ms";
  ...
  if (1920 == frame.cols && 1080 == frame.rows) {
    ...
    double ratio = width_ * 1. / frame.cols;
    int offset_y = (frame.rows - static_cast<int>(frame.rows * ratio)) / 2;
    yolo::recover_bbox(frame.cols, static_cast<int>(frame.rows * ratio), offset_y, objects);
  }
