...
bool load_text_proto_message_file(const std::string& path, google::protobuf::Message& msg) {
    int fd = open(path.c_str(), O_RDONLY);
    if (fd < 0) {
        //AERROR << PERCEPTION_ERROR << CAMERA_ALGORITHM_INIT_CFG_ERROR
    ...
    err_msg  << "path[" << path << "]";
    legacy_check(google::protobuf::TextFormat::Parse(&file_in, &msg), err_msg.str());
    return 0 == close(fd);
}

void legacy_check(bool cond, const std::string &err_msg) {
    if (cond) {
        return;
    }
    AINFO << err_msg;
