...
bool UnityRectify::Init() {
  ...
  x_ = config_.roi_x();
  y_ = config_.roi_y();
  lenth_ = config_.roi_length();
  with_ = config_.roi_width();
...
//  cv::resize(ros_image,ros_image,cv::Size(640,360));
  ...
  cv::Rect cbox(x_,y_,lenth_,with_);
...
//    detect_->Perform(ros_image, &detected_bboxes);
  ...
  for (size_t i = 0; i < lights_ref.size(); ++i) {
    ...
    lights_ref[i]->region.rectified_roi = cbox;//region;
    ...
    lights_ref[i]->region.is_detected = true;//selected_bboxes[i]->region.is_detected;
    lights_ref[i]->region.is_selected = true;//selected_bboxes[i]->region.is_selected;
    AINFO << region;
  }
  return true;
}
