...
__global__ void
set_input_kernel(const unsigned char *src, float *dst, int channel, int height, int width) {
    const int dst_x = blockDim.x * blockIdx.x + threadIdx.x;
    const int dst_y = blockDim.y * blockIdx.y + threadIdx.y;
    if (dst_x < width && dst_y < height) {
        for (int c = 0; c < 3; c++) {
            int dst_idx = (dst_y * width + dst_x) * channel + c;
            dst[dst_idx] = src[dst_idx];
        }
    }
}

void set_input(cv::Mat frame, caffe::Blob<float> *dst) {
#ifdef __x86_64__
    ...
    src_gpu.reset(new caffe::SyncedMemory(width * height * channel * sizeof(unsigned char)));
    src_gpu->set_cpu_data(frame.data);
    set_input_kernel << < grid, block >> > ((const unsigned char *) src_gpu->gpu_data(), dst
        ->mutable_gpu_data(), channel, height, width);
}
