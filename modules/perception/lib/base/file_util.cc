...
bool FileUtil::get_type(const string& filename, FileType* type) {
    ...
    if (S_ISDIR(stat_buf.st_mode) != 0) {
        ...
        *type = TYPE_FILE;
        ...
        AERROR << "failed to get type: " << filename;
        return false;
    }
    return true;
}
...
bool FileUtil::delete_file(const string& filename) {
    ...
    if (dir == NULL) {
        AERROR << "failed to opendir: " << filename;
        return false;
    }
    ...
    while ((dir_info = readdir(dir)) != NULL) {
        ...
        if (!get_type(temp_file, &temp_type)) {
            AERROR << "failed to get file type: " << temp_file;
            closedir(dir);
            return false;
        }
        ...
        if (remove(temp_file.c_str()) != 0) {
            AERROR << "failed to remove file: " << temp_file;
            closedir(dir);
            return false;
        }
    }
...
bool FileUtil::exists(const string& file) {
    ...
    if (ret != 0) {
        AERROR << "file not exist. file: " << file << " ret: " << strerror(errno);
        return false;
    }
    return true;
}
...
bool FileUtil::rename_file(const string& old_file, const string& new_file) {
    ...
    if (ret != 0) {
        AERROR << "failed to rename [old file: " << old_file
...
bool FileUtil::create_dir(const string& dir) {
    ...
    if (ret != 0) {
        AERROR << "failed to create dir. [dir: " << dir
        << "] [err: " << strerror(errno) << "]";
        return false;
    }
    return true;
...
void FileUtil::get_file_list(
    ...
    while (itr != boost::filesystem::recursive_directory_iterator()) {
        try {
            ...
            AERROR << "Caught execption: " << ex.what();
            continue;
        }
    }
}
...
int FileUtil::num_lines(const std::string& filename) {
    std::ifstream ifs(filename.c_str());
    return ifs.good() ?
            count(istreambuf_iterator<char>(ifs), istreambuf_iterator<char>(), '\n') + 1: -1;
}
