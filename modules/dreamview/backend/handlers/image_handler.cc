...
void ImageHandler::OnImage(const sensor_msgs::Image &image) {
  ...
  if (!(image.encoding == "yuyv" || image.encoding == "bgra8" || 
      image.encoding == "bgr8" || image.encoding == "rgb8")) {
  ...
  } else if (image.encoding == "bgra8" || image.encoding == "bgr8" || image.encoding == "rgb8") {
    mat = cv_bridge::toCvCopy(image, sensor_msgs::image_encodings::BGR8)->image;
  }
...
void ImageHandler::OnImageFront(const sensor_msgs::Image &image) {
  if (true/*FLAGS_use_navigation_mode*/) {
    OnImage(image);
  }
}
