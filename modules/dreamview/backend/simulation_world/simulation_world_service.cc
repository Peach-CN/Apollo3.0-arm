...
void SimulationWorldService::Update() {
  ...
  UpdateWithLatestObserved("RelativeMap", AdapterManager::GetRelativeMap(),
                           FLAGS_use_navigation_mode);
  UpdateWithLatestObserved("Ultrasonic", AdapterManager::GetUltrasonic());//??¨¨?3?¨¦¨´2¡§¨ºy?Y
...
template <>
void SimulationWorldService::UpdateSimulationWorld(const Ultrasonic &ultrasonic) {
  world_.mutable_radar_ranges()->Resize(ultrasonic.ranges_size(), 0.0);
  for(int i=0;i<ultrasonic.ranges_size();i++){
    world_.set_radar_ranges(i,ultrasonic.ranges(i));
  }
}
...
void SimulationWorldService::UpdateSimulationWorld(
  ...
  previous_timestamp = topic_timestamp;
