        ...
        if (tabType === 'checkbox') {
            entries = Object.keys(data)
                .map(key => {
                    ...
                    const name = intl.get(item);//获取对应模块的翻译
                    return (
                        <MenuItemCheckbox key={key} id={key} title={name}
                        options={options}/>
                    );
                });
        ...
        const subMenu = Object.keys(menuData)
            .map(key => {
                ...
                if (OFFLINE_PLAYBACK && !item.supportInOfflineView) {
                    ...
                    const name = intl.get(item.title);//获取对应模块的翻译
                    return (
                        <SubMenu key={item.id} tabId={item.id} tabTitle={name}
